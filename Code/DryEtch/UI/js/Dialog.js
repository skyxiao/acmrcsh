...
Dialog.prototype.show = function(){
	...
	var sdw = $(JqueryT("body")[0]).children().outerWidth();
	var sdh = $(JqueryT("body")[0]).children().outerHeight();
	sw=Math.max(sw,cw);
	sh=Math.max(sh,ch) - 140;
	sw = Math.max(sw, sdw);
	sh = Math.max(sh, sdh);
...
if ( !this.ParentWindow ){
	...
	if(this.Top==0){
		this.Top = (ch - this.Height - 30) / 2 + st - 8 - this.Height * 0.1;
	}
	if(this.Left==0){
	...
	window.onresize = function(){
		...
		sw = Math.max(sw, sdw);
		sh = Math.max(sh, sdh);
		$("#" + This.bgdivID).width(sw);
		$("#" + This.bgdivID).height(sh);
	};
}
